...
STATUSLINE_COLS=${STATUSLINE_COLS:-${COLUMNS:-$(tput cols 2>/dev/null || echo 80)}}
# Claude Code indents the bar a little; raise STATUSLINE_MARGIN if the right side wraps.
STATUSLINE_COLS=$((STATUSLINE_COLS - ${STATUSLINE_MARGIN:-4}))

# Display width: strip color codes, then wc -L counts columns (emoji = 2).
# mawk's length() counted bytes, pushing the right side away from the edge.
__visible_len() {
  printf '%s' "$1" | sed 's/\x1b\[[0-9;]*m//g' | LC_ALL=C.UTF-8 wc -L
}
...
__chunk_0="$( {
  ...
  __repeat_char ' ' 6
  if awk -v v="$(__field 'context_window.total_input_tokens')" 'BEGIN{exit !(v+0 > 750000)}'; then
    __v="$(__field 'context_window.used_percentage')"
    __bar_out="$(__bar "$__v" 20 '▰' '▱')"
    __emit '38;5;167' "$__bar_out"
  else
    if awk -v v="$(__field 'context_window.total_input_tokens')" 'BEGIN{exit !(v+0 > 600000)}'; then
      __v="$(__field 'context_window.used_percentage')"
      __bar_out="$(__bar "$__v" 20 '▰' '▱')"
      __emit '38;5;214' "$__bar_out"
    else
      if awk -v v="$(__field 'context_window.total_input_tokens')" 'BEGIN{exit !(v+0 > 450000)}'; then
        __v="$(__field 'context_window.used_percentage')"
        __bar_out="$(__bar "$__v" 20 '▰' '▱')"
        __emit '38;5;226' "$__bar_out"
      else
        __v="$(__field 'context_window.used_percentage')"
        __bar_out="$(__bar "$__v" 20 '▰' '▱')"
        __emit '38;5;114' "$__bar_out"
      fi
    fi
  fi
  __repeat_char ' ' 1
